...
### R Scripts (Census Tract Level)
- `19_epigenetics_constructCTvar.R` - Collect ACS data at census tract level for Massachusetts
- `20_epigenetics_ICE_CT_allUS.R` - Collect ACS data at census tract level for all US states

### Python Script (Block Group Level)
- `collect_acs_blockgroup_data.py` - Collect ACS data at census block group level
  - **Note**: ACS data is NOT available at the census block level (only Decennial Census data is)
  - This script collects data at the **block group** level, which is the smallest geography available for ACS
  - See `CENSUS_GEOGRAPHY_GUIDE.md` for detailed explanation of census geography levels
...
4. Run the script:
   ```bash
   python collect_acs_blockgroup_data.py
   ```

For detailed usage instructions and information about census geography levels, see `CENSUS_GEOGRAPHY_GUIDE.md`. 
